...
module.exports = function(context) {
    ...
    function report(property, side, whitespace, expected) {
        var diff = whitespace.length - expected,
            key = property.key,
            colon = getNextColon(key),
            firstTokenAfterColon = context.getTokenAfter(colon),
            location = side === "key" ? key.loc.start : firstTokenAfterColon.loc.start;

        /**
         * Responsible for fixing the spacing issue
         * @returns {Function} function to be executed by the fixer
         * @private
         */
        function getFixerFunction() {
            if (diff < 0) {
                var spaces = "" + new Array(-diff + 1).join(" ");  // replace with repeat in future

                if (side === "key") {
                    return function(fixer) {
                        return fixer.insertTextBefore(colon, spaces);
                    };
                } else {
                    return function(fixer) {
                        return fixer.insertTextAfter(colon, spaces);
                    };
                }
            } else {
                var rangeToFix = [];
