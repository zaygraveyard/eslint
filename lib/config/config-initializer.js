...
function processAnswers(answers) {
    ...
    if (answers.source === "auto") {
        var start = Date.now();
        var sourceCodes = getSourceCodeOfFiles(answers.patterns, { baseConfig: config });
        autoconfig.determineRules(sourceCodes, config);
    ...
    inquirer.prompt([
        ...
        inquirer.prompt([
            ...
            if (earlyAnswers.source === "auto") {
                ...
                writeFile(config, secondAnswers.format, callback);
                return;
            }
