 ...
 * configurations.
 *
 * The lenght of the returned array will be <= MAX_CONFIG_COMBINATIONS.
 *
...
function buildRuleSets(registry) {
    ...
    function addRuleToRuleSet(rule) {
        if (registry[rule][idx] && registry[rule].length <= MAX_CONFIG_COMBINATIONS) {
            ruleSets[idx] = ruleSets[idx] || {};
            ruleSets[idx][rule] = registry[rule][idx].config;
        }
    }
 ...
 * @returns {Object}               ESLint config object with configured "rules"
 */
function determineRules(sourceCodes, config) {
        ...
        ruleSets.forEach(function(ruleSet) {
            ...
            try {
                ...
                lintResults.forEach(function(result) {
                    registry[result.ruleId][ruleSetIdx].errorCount += 1;
                });
                // Deallocate for GC
                lintResults = null;
            } catch (err) {
    ...
    // 3. Analyze registry to determine which rules can be turned on without errors
